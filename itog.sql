...
select stock_name, min_dt, end_dt, round(student00.st_get_inf_period('RUB', min_dt, end_dt), 6) infl_coeff,
 ...
 order by 11 desc;
 commit;
 select stock_name,min_dt,end_dt,round((sum_div_amt/end_stock_price),2)*100||'%',yearly_avg_incr_over_infl from hp order by 5 desc fetch first 20 rows only; --Chart5
